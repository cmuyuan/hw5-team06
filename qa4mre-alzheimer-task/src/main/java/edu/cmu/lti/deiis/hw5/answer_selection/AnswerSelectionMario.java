...
public class AnswerSelectionMario extends JCasAnnotator_ImplBase {
  ...
  public void process(JCas aJCas) throws AnalysisEngineProcessException {
    ...
    for (int i = 0; i < qaSet.size(); i++) {
      ...
      System.out.println("Question: " + question.getText() + "\tCategory: " + question.getCategory());
      ...
      try {
        bestChoice = findBestChoice(hshAnswer,question.getCategory());
  ...
  public String findBestChoice(HashMap<String, Double> hshAnswer, String category) throws Exception {
    ...
    while (it.hasNext()) {
      ...
      if (category == "quantity"){
        Pattern quantPat = Pattern.compile("[\\d,.]+");
        Matcher quantMat = quantPat.matcher(key);
        if (!quantMat.find()){catMatch = false;}
      } else if (category == "time") {
        Pattern timePat = Pattern.compile("second|minute|hour|day|year|when|before|after|time|now|past|future");
        Matcher timeMat = timePat.matcher(key);
        if (!timeMat.find()){catMatch = false;}
      }
      
      if (category == "quantity"){
        System.out.println(key + "\t" + val + "\tContains quantity: " + catMatch);
      } else if (category == "time") {
        System.out.println(key + "\t" + val + "\tContains temporal: " + catMatch);
      } else {
